...
var Sclicenpato = 0;
var SterrenoC = 0;
var SterrenoI = 0; //cuacks gastados en terrenos
var SartefactoC = 0;
var ScuackP = 0;
var ScuackC = 0;
...
var cuackproduct = [1,3,5,0,-5,200,400,2];
...
var precioartefacto = [500, 3000, 6500, 12000, 15000, 35000, 40000, 80000, 200000, 0];
...
function actualizarproduccion(){
	...
	for (i=0; i<terreno.length; i++){
		if(i==3){
			cuackseg += Math.round(parseInt(2 * terreno[i] * terreno[1]));
...
function render(){
	...
	if (coronaOlimpo){
		document.getElementById("cuackseg").innerHTML = "Producción: <span>" + (cuackseg*3) + " Cuacks/s </span>";
	}else{
	...
	if(ext){
		document.getElementById("poderclic").innerHTML = "Poder de click: <span>" + (poderclic + Math.round(parseInt(cuackseg/75)))+ "</span>"; 
	}
	//estadísticas
				  ...
				  function frame() {
				    if (width >= 100) {
				      ...
				      if(poderejercito>=5000){
						...
						sello = true;
						alert("¡Felicidades! ¡Conseguiste el Sello de la victoria y has completado Clicking duck! Aún así, puedes seguir jugando para conseguir mas Cuacks")
...
function reclamaRecompensa(num){
	switch (num) {
    	...
    	case 2:
    		$("#rCascada").hide();
    		document.getElementById("botonCascada").disabled = false;
    	break;
    	case 3:
			$("#rCueva").hide();
    		document.getElementById("botonCueva").disabled = false;
    	break;
    	case 4:
    		$("#rMontania").hide();
    		document.getElementById("botonMontania").disabled = false;
    	break;
  		case 5:
    		$("#rVolcan").hide();
...
function reclamar(numero){
	switch (numero) {
  		case 0:
  		if(terreno[0]>=10){
    		cuack+=800;
    		document.getElementById("rec0").disabled = true;
    	}
    	break;
    	case 1:
    	if(terreno[1]>=10){
    	...
    	break;
    	case 7:
    	if(terreno[0]>0 && terreno[1]>0 && terreno[2]>0 && terreno[3]>0 && terreno[5]>0 && terreno[6]>0){
    		cuack+=22222;
    		document.getElementById("rec7").disabled = true;
    		}
    	break;
    	case 8:
    	if(Sclicenpato>=1000){
    		cuack+=5001;
    		document.getElementById("rec8").disabled = true;
    		}
    	break;
    	case 9:
    	if((ScuackP + ScuackC + ScuackCosechadora)>=1000000){
			cuack+=50000;
			document.getElementById("rec9").disabled = true;
			}
    	break;
    	case 10:
